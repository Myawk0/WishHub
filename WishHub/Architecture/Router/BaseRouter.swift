...
class BaseRouter: NSObject {
    ...
    func back(over: Int = 0, animated: Bool = true, completion: (() -> ())? = nil) {
        removePreviousScreens(over)
        navigation.dismissOrPop(animated: animated, completion)
    }
    
    func open(viewController: UIViewController, removeCurrent: Bool = false, animated: Bool = true, _ completion: (() -> ())? = nil) {
        navigation.presentOrPush(viewController: viewController, removeCurrent: removeCurrent, animated: animated, completion)
    }
    
    func openAsBottomSheet(viewController: UIViewController, _ completion: (() -> ())? = nil) {
    ...
    func removePreviousScreens(_ count: Int, removeCurrent: Bool, andOpen vc: BaseViewController) {
        for _ in 0..<count {
            removePreviousScreen()
        }
        navigation.presentOrPush(viewController: vc, removeCurrent: removeCurrent, animated: true, nil)
    }
